# --------------------------------------------------------------
# Dockerfile for ESS‑helm on Railway (clone‑then‑template approach)
# --------------------------------------------------------------

# 1️⃣ Start from a tiny Alpine base
FROM alpine:3.20
...
RUN apk add --no-cache \
        ...
        ca-certificates \
        git && \
    # 3️⃣ Download and install Helm (v3.14.0 at the time of writing)
...
# -----------------------------------------------------------------
# 4️⃣ Clone the ESS‑helm chart source code
# -----------------------------------------------------------------
WORKDIR /chart
RUN git clone https://github.com/kimsokhom/ess-helm.git . \
    && git checkout main   # you can replace `main` with a tag/commit if you wish

# -----------------------------------------------------------------
# 5️⃣ Bring in your custom values file (keep it in the same repo)
# -----------------------------------------------------------------
...
# -----------------------------------------------------------------
# 6️⃣ Render the chart and keep the container alive
# -----------------------------------------------------------------
ENTRYPOINT ["bash", "-c", "\
